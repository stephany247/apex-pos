...
import { TrendingUp, DollarSign, ShoppingBag, Users, ArrowUpRight, ArrowDownRight, MoreHorizontal } from 'lucide-react';
...
const ReportsView: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    const chartData = Object.keys(salesByDay).map(date => ({ date, sales: salesByDay[date] }));
    const pieData = Object.keys(salesByMethod).map(method => ({ name: method, value: salesByMethod[method] }));
      ...
      <div className="flex items-center justify-end">
         <div className="bg-white rounded-full p-1 border border-zinc-100 flex gap-1">
            {['Day', 'Week', 'Month'].map(t => (
                <button key={t} className={`px-4 py-1.5 rounded-full text-sm font-bold transition-all ${t === 'Week' ? 'bg-[#111] text-white' : 'text-zinc-500 hover:text-zinc-900'}`}>{t}</button>
            ))}
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-8 rounded-[2rem] shadow-sm border border-zinc-100">
            <div className="flex justify-between items-center mb-8">
                ...
                <button className="p-2 rounded-full hover:bg-zinc-50 text-zinc-400"><MoreHorizontal /></button>
            </div>
            <div className="h-72">
...
const KPICard: React.FC<KPICardProps> = ({ title, value, icon, trend, bgColor, textColor, borderColor = '' }) => {
    const isPositive = trend.startsWith('+');
    return (
        <div className={`${bgColor} ${textColor} p-6 rounded-[2rem] relative overflow-hidden transition-transform hover:-translate-y-1 duration-300 ${borderColor}`}>
            <div className="flex justify-between items-start mb-8 relative z-10">
                <div className="p-3 bg-black/5 rounded-full backdrop-blur-sm">{icon}</div>
