...
import { Search, AlertTriangle, PackageCheck, Pencil, Save, X, Filter, Package, Smartphone, Shirt, Coffee, Home, Sparkles, Plus } from 'lucide-react';
import { Product, ProductCategory } from '../../types';

// Helper for category colors and icons
const getCategoryStyles = (category: string) => {
    switch(category) {
        case 'Electronics': return { bg: 'bg-blue-50', text: 'text-blue-600', icon: <Smartphone size={20} /> };
...
const InventoryView: React.FC = () => {
  ...
  const saveEdit = (id: string) => {
      ...
      const timestamp = selectedDate ? new Date(selectedDate).toISOString() : undefined;
  ...
  const handleAddProduct = (e: React.FormEvent) => {
      ...
      setNewProduct({ category: 'Electronics', stock: 0, price: 0, cost: 0, lowStockThreshold: 5 });
  };
      ...
      <div className="flex-1 overflow-auto px-6 pb-6">
        ...
        <table className="w-full text-left border-collapse">
            <thead className="bg-[#F4E6CB]">
                <tr>
                    <th className="p-5 font-bold text-zinc-900 text-sm">Product</th>
                    <th className="p-5 font-bold text-zinc-900 text-sm">SKU / Last Updated</th>
                    <th className="p-5 font-bold text-zinc-900 text-sm">Category</th>
                    <th className="p-5 font-bold text-zinc-900 text-sm">Price</th>
                    <th className="p-5 font-bold text-zinc-900 text-sm text-center">Stock</th>
            ...
            <tbody className="divide-y divide-zinc-50">
                {filteredProducts.map(product => {
                    ...
                    return (
                    <tr key={product.id} className="hover:bg-zinc-50/50 group transition-colors">
                        <td className="p-5">
                            <div className="flex items-center gap-4">
                                <div className={`w-12 h-12 rounded-xl ${styles.bg} ${styles.text} flex items-center justify-center`}>
                                    {styles.icon}
                                </div>
                                <span className="font-bold text-zinc-800 text-sm">{product.name}</span>
                            </div>
                        </td>
                        <td className="p-5">
                            <div className="text-zinc-500 font-mono text-xs font-bold">{product.sku}</div>
                            {product.lastUpdated && (
                        ...
                        <td className="p-5 font-medium text-zinc-800 text-sm">₦{product.price.toFixed(2)}</td>
                        <td className="p-5 text-center">
                            {editingId === product.id ? (
                                <div className="flex items-center justify-center gap-2 bg-white shadow-lg p-1 rounded-full border border-zinc-100 inline-flex">
                                    <button onClick={() => setEditValue(Math.max(0, editValue - 1))} className="w-8 h-8 flex items-center justify-center bg-zinc-100 rounded-full hover:bg-zinc-200 font-bold">-</button>
                                    <input 
                                        type="number" 
                                        value={editValue} 
                                    ...
                                    />
                                    <button onClick={() => setEditValue(editValue + 1)} className="w-8 h-8 flex items-center justify-center bg-zinc-100 rounded-full hover:bg-zinc-200 font-bold">+</button>
                                </div>
                            ) : (
                                <span className={`
                                    inline-flex items-center px-3 py-1 rounded-full text-xs font-bold
                                    ${product.stock === 0 ? 'bg-red-100 text-red-800' : 
                                      product.stock <= product.lowStockThreshold ? 'bg-[#FDE047] text-black' : 
                        ...
                        <td className="p-5 text-right">
                            {editingId === product.id ? (
                                <div className="flex items-center justify-end gap-2">
                                    <button onClick={() => saveEdit(product.id)} className="p-2 bg-black text-white rounded-full hover:scale-105 transition-transform"><Save size={16} /></button>
                                    <button onClick={() => setEditingId(null)} className="p-2 bg-zinc-100 text-zinc-500 rounded-full hover:bg-zinc-200"><X size={16} /></button>
                                </div>
                            ) : (
                                <button onClick={() => startEdit(product)} className="p-2 text-zinc-300 hover:text-black hover:bg-zinc-100 rounded-full transition-all">
                                    <Pencil size={18} />
                                </button>
                            )}
                        </td>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto custom-scrollbar">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-2xl font-bold text-zinc-900">Add New Product</h3>
                    <button onClick={() => setShowAddModal(false)} className="p-2 hover:bg-zinc-100 rounded-full transition-colors"><X size={20} /></button>
                </div>
                ...
                <form onSubmit={handleAddProduct} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <label className="text-sm font-bold text-zinc-700 ml-1">SKU</label>
                            <input 
                                required
                                type="text" 
                                className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-black"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1">
                            <label className="text-sm font-bold text-zinc-700 ml-1">Cost (₦)</label>
                            <input 
                                required
                                type="number" 
                                min="0"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <label className="text-sm font-bold text-zinc-700 ml-1">Initial Stock</label>
                            <input 
                                required
                                type="number" 
                                min="0"
                        ...
                        <div className="space-y-1">
                            <label className="text-sm font-bold text-zinc-700 ml-1">Low Stock Alert</label>
                            <input 
                                required
                                type="number" 
                                min="0"
