...
const Cart: React.FC = () => {
  const { cart, removeFromCart, updateCartQuantity, clearCart, completeSale } =
    useStore();
  const [showPayment, setShowPayment] = useState(false);
  const [paymentStep, setPaymentStep] = useState<"method" | "confirm">(
    "method",
  );
    ...
    <div className="flex flex-col h-full bg-white rounded-[2rem] shadow-sm border border-zinc-100 overflow-hidden relative">
      ...
      <div className="p-6 pb-4 border-b border-zinc-50">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={clearCart}
            className="text-xs font-medium text-red-500 hover:text-red-700 px-2 py-1 bg-red-50 rounded-lg transition-colors"
          >
            Clear
          </button>
        </div>
        ...
        {cart.length === 0 ? (
          ...
          cart.map((item) => {
            ...
            return (
              <div
                key={item.cartId}
                className="flex gap-3 bg-white p-2 rounded-2xl group hover:bg-zinc-50 transition-colors"
              >
                <div
                  className={`w-14 h-14 rounded-xl ${styles.bg} ${styles.text} flex items-center justify-center flex-shrink-0`}
                >
                  {styles.icon}
                </div>

                <div className="flex-1 flex flex-col justify-center">
                  <h4 className="font-bold text-zinc-800 text-sm line-clamp-1 capitalize">
                    {item.name}
                  </h4>
      ...
      {/* Totals & Actions */}
      <div className="p-6 pt-3 bg-zinc-50/50 border-t border-zinc-100 backdrop-blur-sm">
        ...
        <div className="fixed inset-0 bg-[#111]/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden p-6 relative">
            <button
              ...
              className="absolute top-6 right-6 p-2 bg-zinc-50 rounded-full hover:bg-zinc-100 transition-colors z-10"
            >
              <X size={20} />
            </button>
              ...
              <>
                <div className="mb-6">
                  <button
                    ...
                    className="flex items-center gap-2 text-zinc-500 hover:text-black transition-colors mb-4 text-sm font-semibold"
                  >
      ...
      {showReceipt && lastTransaction && (
        <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white w-full max-w-sm rounded-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div
              className="p-8 overflow-y-auto bg-white text-zinc-800 font-mono text-sm leading-relaxed"
              id="receipt-content"
            >
...
const PaymentOption = ({ icon, title, subtitle, onClick, color }: any) => (
  <button
    onClick={onClick}
    className="w-full flex items-center gap-4 p-4 border border-zinc-100 rounded-2xl hover:border-black/10 hover:shadow-lg transition-all text-left group bg-white"
  >
    <div
