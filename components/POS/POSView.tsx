...
import { Search, Grid, List, Plus, ScanBarcode, Filter, Package, Tag, ShoppingBag, Smartphone, Shirt, Coffee, Home, Sparkles } from 'lucide-react';
...
const POSView: React.FC = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full gap-4 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-full bg-white rounded-[2rem] shadow-sm border border-zinc-100 overflow-hidden relative">
        {/* Search & Filter Header */}
        <div className="p-5 pb-3 space-y-3">
          <div className="flex gap-3">
            <div className="relative flex-1 group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-zinc-800 transition-colors" size={20} />
              <input
                type="text"
              ...
              <button
                ...
                className={`
                  ...
                  ${selectedCategory === cat 
                    ? 'bg-[#111] text-white shadow-lg shadow-zinc-900/20' 
          ...
          {viewMode === 'grid' ? (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
             ...
             <div className="space-y-2">
              {filteredProducts.map(product => (
                <ProductListItem key={product.id} product={product} onAdd={() => addToCart(product)} />
              ))}
            </div>
          )}
...
export const getCategoryStyles = (category: string) => {
    switch(category) {
        case 'Electronics': return { bg: 'bg-blue-50', text: 'text-blue-600', icon: <Smartphone size={24} /> };
        case 'Clothing': return { bg: 'bg-purple-50', text: 'text-purple-600', icon: <Shirt size={24} /> };
        case 'Groceries': return { bg: 'bg-green-50', text: 'text-green-600', icon: <Coffee size={24} /> };
        case 'Home': return { bg: 'bg-orange-50', text: 'text-orange-600', icon: <Home size={24} /> };
        case 'Beauty': return { bg: 'bg-pink-50', text: 'text-pink-600', icon: <Sparkles size={24} /> };
        default: return { bg: 'bg-zinc-50', text: 'text-zinc-600', icon: <Package size={24} /> };
    }
}
...
const ProductCard: React.FC<{ product: Product; onAdd: () => void }> = ({ product, onAdd }) => {
  ...
  return (
    <button
      ...
      `}
    >
      <div className={`relative aspect-[4/3] w-full overflow-hidden rounded-2xl ${styles.bg} ${styles.text} flex items-center justify-center mb-3 transition-colors group-hover:brightness-95`}>
      ...
      <div className="flex flex-col flex-1 px-1">
        <h3 className="font-bold text-zinc-800 leading-snug line-clamp-2 mb-1 group-hover:text-black text-sm">{product.name}</h3>
...
const ProductListItem: React.FC<{ product: Product; onAdd: () => void }> = ({ product, onAdd }) => {
    const styles = getCategoryStyles(product.category);
    return (
        <button 
            onClick={onAdd}
        ...
        >
            <div className={`w-12 h-12 rounded-2xl ${styles.bg} ${styles.text} flex items-center justify-center`}>
            ...
            <div className="flex-1">
                ...
                <p className="text-[10px] text-zinc-500 mt-0.5">{product.sku} • Stock: {product.stock}</p>
            </div>
